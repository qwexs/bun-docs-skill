...
description: >
  ...
  bun:sqlite, Bun.s3, Bun.redis, Bun.Archive), CLI (bun install, bun test
  --parallel, bun run --parallel, bun audit fix, bun build, bunx),
  bunfig.toml, Node-to-Bun replacements, or "what's new in Bun".
  Triggers: bun, bun.sh, bun.com, @types/bun, Bun.Image, Bun.WebView,
  bun test. Use when the user runs /bun-docs.
---
...
5. For "what landed in X" or upgrading: `https://bun.com/blog/bun-vX.Y.Z` and
   `bun --version`. Breaking changes live in that post's Upgrading section.
...
- Mark experimental APIs as experimental when you recommend them: HTTP/2
  and HTTP/3 `fetch` clients, `Bun.serve({ http3 })`, `install.globalStore`,
  Android builds.
...
3. Changelog: https://bun.com/blog/bun-v1.4.0
